...
1. Edit `molecule/hello-delegated/create.yml` to add the tofu project setup and `community.FIXME.terraform` blocks.
1. Edit `molecule/hello-delegated/destroy.yml` to add the `community.FIXME.terraform` block.
...
- `tofu_project_repo` - The url of the git repo containing the tofu project. The example points at the `example` branch of my molecule delegate project. Used by the `community.FIXME.terraform` tasks.
- `tofu_project_version` - The branch/ref/tag of the tofu project repo to checkout. Used by the `community.FIXME.terraform` tasks.
